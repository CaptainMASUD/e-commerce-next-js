...
const TOP_FEATURES = [
  { label: "Secure Payment", Icon: FiShield, tint: "rgba(0,31,63,.06)", dot: PALETTE.gold },
  { label: "Faster Delivery", Icon: FiTruck, tint: "rgba(255,126,105,.10)", dot: PALETTE.coral },
  { label: "Easy Returns", Icon: FiRefreshCcw, tint: "rgba(234,179,8,.10)", dot: PALETTE.gold },
  { label: "Premium Support", Icon: FiAward, tint: "rgba(0,31,63,.06)", dot: PALETTE.cta },
];

export default function FeatureStrips() {
  return (
    <section className="mt-2">
      <div className="rounded-[1.25rem] border border-black/5 bg-white p-1.5 shadow-sm sm:rounded-[1.75rem] sm:p-4">
        <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-4 sm:gap-3">
          {TOP_FEATURES.map(({ label, Icon, tint, dot }) => (
            <div
              key={label}
              className={cn(
            ...
            >
              ...
              >
                ...
                <span
                  className={cn(
                    "pointer-events-none absolute -right-0.5 -top-0.5 rounded-full ring-2 ring-white",
                    "h-1 w-1 sm:-right-1 sm:-top-1 sm:h-2.5 sm:w-2.5"
                  )}
